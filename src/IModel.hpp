...
#define RegisterModel(model_name, new_model, T)                                                                        \
    ...
    public:
        ...
        /// функция вернет модель из хранилища. Любое взаимодействие с моделью должно производиться ТОЛЬКО ПОСЛЕ
        /// РЕГИСТРАЦИИ, иначе все изменения могут быть утеряны
        template<class ConcreteModel>
        ConcreteModel *registerModel(ConcreteModel *model) {
            auto it = find(model->name());
            if (it != end()) {
                delete model;

                Y_LOG(0, "Model already registered :" << model->name().c_str() << "\n");
                return static_cast<ConcreteModel *>(it->second);
            } else {
                model->init();
                insert({model->name(), model});
                regSeq_.push_back(model);
                G_LOG(0, "Model registered success [Delete order " << model->deleteOrder() << "] " << model->name().
                      c_str());
                return model;
            }
        }
        ...
        template<class ConcreteModel>
        ConcreteModel *getModel(const model_name &name) {
            if (const auto it = find(name); it != end()) return static_cast<ConcreteModel *>(it->second);
            return nullptr;
        }

        bool contains(const model_name &name) { return find(name) != end(); }
